...
const renderAlertList = () => {
    ...
    if (list) {
        ...
        arrayList.forEach(alert => {
            ...
            el.innerHTML += `
            <h1 class="font-semibold">${alert[0].charAt(0).toUpperCase() + alert[0].slice(1)} -- $${alert[1].toLocaleString('en-US')}</h1>
...
const renderCryptoCards = (arr) => {
    ...
    arr.forEach(coin => {
        ...
        card.querySelector('.price .main_price').textContent = `$${Math.ceil(coin.current_price).toLocaleString('en-US')}`;
    ...
    if (list) {
        ...
        arrayList.forEach(alert => {
            ...
            if (targetPrice > currentPrice) {
                notify(`${Name}'s current Price is ${Math.ceil(currentPrice)}$, and Your Target was ${targetPrice}$, Target Reached! You can invest Now!`);
                setTimeout(() => {
                    delete alerts[Name];
                    localStorage.setItem('crypto_alerts', JSON.stringify(alerts));
                    renderAlertList();
